...
The presence of considerable heritable variation in flight performance across segregants clearly shows that there is sufficient variation for selection to act upon (Table \ref{tab:cvr}).  While the relationship between flight phenotypes and fitness is not simple, it is likely that higher takeoff angles, velocity and acceleration allow for improved fitness.  Higher performance in all of these metrics in turn require greater energy consumption and higher metabolic output.

To determine whether energetic requirements of flight are paralleled in metabolic variation, we sought to determine if these Drosophila lines differed in either the metabolic capacity of enzymes or patterns of pathway utilization.  While associating flight with its energetic determinants, an optimal approach would be to compare in-flight metabolism and gas-exchange with flight phenotypes.  Because of difficulties in sustaining the flight of a large number of \textit{Drosophila} for respirometry or enzyme kinetic experiments, measurements on cohorts of resting flies were taken, noting the caveat that associations with flight will be found only to the extent that variation across resting metabolic states can be treated as proxy for variability in flight metabolism.  In producing this sort of model, our goal was to integrate both previously measured enzyme activities \cite{Greenberg:2011jf} as well as direct gas-exchange fluxes.  These gas-exchange rates, of CO$_{2}$ production and O$_{2}$ consumption are an important metabolic constraints as they are indicative of the overall flux through the electron transport chain as well as the net levels of biomolecule production or breakdown relative to energy generation.   
...
In contrast to flight traits, where between-population variation was minimal, there is pronounced variation between populations in gas-exchange traits (Figure \ref{linetraitvar}a-c).  Oxygen consumption is depressed in Beijing lines (despite them being the heaviest population), and this low oxygen consumption elevates the respiratory quotient.  For all gas-exchange traits, there is ample heritable variation within each population (Table \ref{tab:cvr}).  
